...
- `content`: `caption` ≤ 5000, `hashtags` ≤ 1000 (tek metin, `#` ile), `videoTitle` ≤ 300, `shortTitle` ≤ 300, `thumbPrompt` ≤ 5000, `slidePrompts` (carousel; en çok 9 metin), `timezone` (IANA, ör. `Europe/Istanbul`; boşsa üreticinin dilimi), `anaDil` (ana metinlerin dili, iki harf, ör. `tr`) ve `diller` (aynı paylaşımın başka dillerdeki hâli: `{"en": {"videoTitle": …, "caption": …, "hashtags": …, "shortTitle": …}}`, en çok 8 dil, iki harfli kod). Çok dilli başlık yalnızca YouTube uzun video ve Shorts'ta anlamlı; diğer platformlarda farklı dil ayrı paylaşım demek. Bir de `hesap`: üreticinin o platformdaki kendi hesaplarından hangisine gideceği, **adıyla** (ör. `"hesap": "Kişisel"`). Yalnızca aynı platformda birden çok hesabı olanlar için anlamlı. Bu bir etikettir, bağlantı değil — Shootboard hiçbir hesaba giriş yapmaz ve hiçbir yere gönderi atmaz. Tanınmayan ad yok sayılır.
