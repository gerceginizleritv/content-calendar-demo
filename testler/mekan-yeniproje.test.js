...
(async () => {
  ...
  bak('mekan secimi penceresinde var', await p.$('#pe_placeSecenek') !== null);
  bak('arama kutusu var', await p.$('#pe_placeAra') !== null);
  const satirlar = await p.$$eval('#pe_placeSecenek [data-mk-sec]', e=> e.map(x=> x.dataset.mkSec));
  bak('mekanlar listeleniyor', satirlar.indexOf('m_kariye') !== -1 && satirlar.indexOf('m_ev') !== -1, satirlar.join(','));
  bak('"+ Yeni mekan" dugmesi var', await p.$('#pe_placeYeni') !== null);
  bak('baslangicta hicbiri secili degil',
      await p.$$eval('#pe_placeSecenek .secili', e=> e.length) === 0);

  console.log('[arama]');
  await p.fill('#pe_placeAra', 'kariye');
  await p.waitForTimeout(150);
  bak('arama listeyi suzuyor',
      await p.$$eval('#pe_placeSecenek [data-mk-sec]', e=> e.map(x=> x.dataset.mkSec)).then(x=> x.join(',')) === 'm_kariye');
  await p.fill('#pe_placeAra', 'Fatih');
  await p.waitForTimeout(150);
  bak('ILCEYE gore de araniyor',
      await p.$$eval('#pe_placeSecenek [data-mk-sec]', e=> e.map(x=> x.dataset.mkSec)).then(x=> x.indexOf('m_kariye') !== -1));
  await p.fill('#pe_placeAra', 'zzz');
  await p.waitForTimeout(150);
  bak('eslesme yoksa soyluyor', /E\u015fle\u015fen|match/i.test(await p.$eval('#pe_placeSecenek', e=> e.textContent)));
  await p.fill('#pe_placeAra', '');
  await p.waitForTimeout(150);

  console.log('[secim]');
  await p.click('[data-mk-sec="m_kariye"]');
  await p.waitForTimeout(150);
  const bilgi = await p.$eval('#pe_placeBilgi', e=> e.textContent);
  ...
  bak('dikkat varsa satir renkleniyor',
      await p.$eval('#pe_placeBilgi', e=> e.className.indexOf('mk-uyari') !== -1));
  bak('kutucuk isaretlendi',
      await p.$eval('[data-mk-sec="m_kariye"]', e=> e.className.indexOf('secili') !== -1));
  // COKLU SECIM: ikinci mekan ilkini SILMIYOR, listeye ekleniyor.
  await p.click('[data-mk-sec="m_ev"]');
  await p.waitForTimeout(150);
  bak('ikinci mekan eklendi, ilki duruyor',
      await p.$$eval('#pe_placeList [data-cip]', e=> e.map(x=> x.dataset.cip)).then(x=> x.join(',')) === 'm_kariye,m_ev');
  bak('notsuz mekanda uyari rengi kalkiyor',
      await p.$eval('#pe_placeBilgi', e=> e.className.indexOf('mk-uyari') === -1));
  // Tekrar tiklamak CIKARIYOR.
  await p.click('[data-mk-sec="m_ev"]');
  await p.waitForTimeout(150);
  bak('tekrar tiklayinca cikiyor',
      await p.$$eval('#pe_placeList [data-cip]', e=> e.map(x=> x.dataset.cip)).then(x=> x.join(',')) === 'm_kariye');

  console.log('[adres alani]');
  bak('mekan secilince adres alani GIZLI',
      await p.$eval('#pe_addressWrap', e=> e.hidden));
  await p.click('#pe_placeList [data-cip="m_kariye"] [data-cip-sil]');
  await p.waitForTimeout(150);
  await p.selectOption('#pe_type', 'venue');
  await p.waitForTimeout(150);
  bak('mekan kalmayinca adres alani geri geliyor',
      await p.$eval('#pe_addressWrap', e=> e.hidden) === false);

  console.log('[olusturma]');
  await p.click('[data-mk-sec="m_kariye"]');
  await p.fill('#pe_name', 'Kariye — bölüm 1');
  ...
  await p.click('#p_openNew');
  await p.waitForSelector('#projectEditOverlay.open');
  bak('secim sifirlandi', await p.$$eval('#pe_placeSecenek .secili', e=> e.length) === 0);
  bak('arama kutusu da sifirlandi', await p.$eval('#pe_placeAra', e=> e.value) === '');
  bak('bilgi satiri ipucuna dondu',
      /mekan|Mekan/.test(await p.$eval('#pe_placeBilgi', e=> e.textContent)));
  ...
  const yeni = await p.evaluate(()=>({
    duraklar: projeMekanSecimiOku('pe'),
  ...
  bak('yeni mekan PROJEYE de ekleniyor',
      !!yeni.kayit && yeni.duraklar.indexOf(yeni.kayit.id) !== -1, JSON.stringify(yeni.duraklar));
  bak('bilgi satirinda yeni mekanin adresi',
      /Sultanahmet, Fatih/.test(await p.$eval('#pe_placeBilgi', e=> e.textContent)));
  ...
  await p.waitForFunction(()=> !document.getElementById('placeOverlay').classList.contains('open'));
  const oncekiDuraklar = await p.evaluate(()=> projeMekanSecimiOku('pe').join(','));
  bak('vazgecilince secim degismiyor', oncekiDuraklar === 'm_kariye,' + yeni.kayit.id, oncekiDuraklar);
  ...
  bak('sonradan eklenen mekan forma sicramiyor',
      await p.evaluate(()=> projeMekanSecimiOku('pe').join(',')) === oncekiDuraklar);

  console.log('[duzenleme penceresi bozulmadi]');
  await p.click('#pe_cancel');
  await p.waitForTimeout(150);
  await p.evaluate(()=>{ openProjectEdit(projects[0].id); });
  await p.waitForSelector('#projectEditOverlay.open');
  bak('duzenlemede mekan hala secili',
      await p.$$eval('#pe_placeList [data-cip]', e=> e.map(x=> x.dataset.cip)).then(x=> x.join(',')) === 'm_kariye');
