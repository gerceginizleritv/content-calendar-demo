...
(async () => {
  ...
  for (const tema of ['light','dark']) {
    ...
    const r = await page.evaluate(async ()=>{
      ...
      const kur = ad=>{ document.getElementById('p_type').value='studio';
        document.getElementById('p_type').dispatchEvent(new Event('change'));
        document.getElementById('p_name').value=ad; document.getElementById('p_add').click(); };
      kur('Kariye Mozaikleri'); kur('Balat Sokakları'); kur('Sirkeci Garı');
