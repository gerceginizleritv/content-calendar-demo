...
(async () => {
  ...
  bak('proje penceresinde mekan seçimi var', await p.$('#pe_placeSecenek') !== null);
  const secenek = await p.$$eval('#pe_placeSecenek [data-mk-sec]', e => e.map(x=>x.textContent.replace(/\s+/g,' ').trim()));
  bak('mekanlar listede', secenek.some(x=> /Kariye (Müzesi|Museum)/.test(x)), secenek.join(' | '));
  bak('şehir/ilçe satırda da yazıyor', secenek.some(x=>x.includes('Fatih')));
  const mid = await p.evaluate(() => mekanlar.find(x=>x.name==='Kariye Müzesi').id);
  await p.click(`[data-mk-sec="${mid}"]`);
